...
{
    public partial class SalesOutputGrowth : Form
    {
        List<OutputInfo> recivedRawList;

        public SalesOutputGrowth(List<OutputInfo> recivedRawList)
        {
            this.recivedRawList = recivedRawList;
            InitializeComponent();

            OutputInfo temp = recivedRawList[0];
            lbl_title.Text = temp.GetOutputName() + "의 출고가 성장도"; // 타이틀 레이블 초기화
        ...
        private List<string> Grouping(string standard)
        {
            ...
            foreach (var i in recivedRawList) // 그룹핑 작업
            {
                ...
                if (standard == "년")
                {
                    ...
                    key = $"{i.GetOutputName()}_{date.Year}"; // 키에 년 설정
                    groupName = $"{date.Year}년";
                }
                else if (standard == "월")
                {
                    DateTime date = DateTime.Parse(i.GetOutputDate());
                    key = $"{i.GetOutputName()}_{date.Year}_{date.Month:D2}"; // 키에 년, 월 설정
            ...
            {
                OutputInfoRow entry = groupedData[key];

                int difference = entry.GetOutputPrice(); // 현재 그룹의 출고가

                int change = difference - previousDifference; // 전 그룹 대비 출고가 차이값
                previousDifference = difference;

                string formattedGroupName = entry.GetOutputGroup();
